<?php
  ...
  function createUser($fname, $username, $email, $lvllist){
    ...
    $message = 'Hi '.$fname.', your account has just been created.  Your username is '.$username.' and your password is '.$password.'';
  ...
  function editUser($id, $fname, $username, $password, $email) {
    ...
    if ($password == 'Leave Blank If You Would Like To Keep The Same Password'){
      $updatestring = "UPDATE tbl_users SET user_fname='{$fname}', user_name='{$username}', user_email='{$email}', user_verified = 'yay' WHERE user_id={$id}";
  		$updatequery = mysqli_query($link, $updatestring);

    }else{
      $newencryptedpass = password_hash($password, PASSWORD_DEFAULT);
  		$updatestring = "UPDATE tbl_users SET user_fname='{$fname}', user_name='{$username}', user_pass='{$newencryptedpass}', user_email='{$email}', user_verified = 'yay' WHERE user_id={$id}";
  		$updatequery = mysqli_query($link, $updatestring);
    }
